...
function SendReminderForm({ userId, handleClose }: { userId: number, handleClose: () => void }) {
    ...
    const filteredTickets = tickets.reduce((acc, ticket) => {
        const foundOwesPayment = owesPayments.filter((owesPayment) => owesPayment.forTicketId === ticket.id && owesPayment.payerId === userId)
        const foundPayments = payments.filter((payment) => foundOwesPayment.some((owesPayment) => owesPayment.id === payment.forOwesPaymentId))
        const paymentsSum = foundPayments.reduce((acc, payment) => acc + payment.amount, 0)
                        ...
                        <div className="space-y-6 sm:space-y-5">
                            ...
                            <div className="space-y-6 sm:space-y-5">
                                ...
                                <ul role="list" className="divide-y divide-gray-200">
                                    ...
                                    {Object.entries(filteredTickets).map(([ticketId, { ticket, remainingAmount }]) => (
                                        <li key={ticketId}>
                                            <a
